...
import { setStoreConfiguration, setStoreConfigurationLoading } from '@/Redux/StoreConfiguration/StoreConfigurationSlice';

export const getPublicStoreConfiguration = async (storeId) => {
  dispatch(setStoreConfigurationLoading(true));

  try {
    const { data } = await axios.get(`${BASE_URL}/${storeId}/getPublicConfiguration`);
