...
const StoreDetails = ({ onClose, onComplete }) => {
  ...
  const validate = () => {
    ...
    if (formData.referralCode && typeof formData.referralCode !== 'string') {
      errs.promoCode = 'Promo code must be a string';
    }
  ...
  return (
    <BackgroundFrame>
      <ActionCard
        ...
        className="p-[30px]"
        actions={<Button label="Generate Another" action={() => setShowModal(true)} size="small" variant="" icon={<FaArrowRightLong />} iconPosition="right" iconOnHover={true} />}
        actionPosition="top"
      >
        {allStores.length > 0 &&
          allStores.map((store, index) => (
            <Link key={index} href={`/admin/${store?._id}`} className="flex items-center gap-3 p-4 rounded-md hover:bg-gray-100 transition cursor-pointer group">
      ...
      {/* Modal */}
      <Modal isOpen={showModal} setIsOpen={() => setShowModal(false)} position="fixed bg-opacity-30" className="!max-w-[550px]">
        <ActionCard
