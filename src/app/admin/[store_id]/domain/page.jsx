...
const DomainVerification = () => {
  ...
  return (
    <div className="flex flex-col gap-[20px] py-[20px] min-h-screen px-8">
      <ActionCard
        label={'Sub Domain'}
        subText={'View your sub domain.'}
        className={'relative'}
      >

        <p className="text-[18px] text-textC">Your Site subdomain is <Link href={`${HTTP}${store?.subDomain}.${Base_Domain}`} target={'blank'} className="text-[#386ec5] hover:opacity-80">{store?.subDomain}.{Base_Domain}</Link></p>
      ...
      {!store?.customDomain ? (
        ...
        >
          <ActionCard
            ...
            <div className="w-full h-full absolute top-0 left-0 bg-[#ffffffa0] backdrop-blur-md z-[20] rounded-md flex flex-col gap-4 justify-center items-center" >
              <p className="text-textC font-medium flex items-center gap-3 text-[22px]"><CiUnlock strokeWidth={1.3} size={20} /> Unlock Custom Domain Setup.</p>
              <Button
                size="small"
                label="Upgrade Now"
              />
        ...
        <ActionCard label={'Custom Domain'} error={error} loading={loading} actions={
          <>
            <Button size="small" icon={<IoReloadOutline />} iconPosition="right" label="Verify" loading={loading} variant="white" action={() => handleVerifyDomain()} className="w-max" />
            <Button size="small" label="Delete Domain" loading={deleteLoading} variant="danger" action={() => { handleDeleteDomain() }} className="w-max" />
          </>
        }>

          {!status?.verified && (
            <div className="flex flex-col gap-3">
              {status?.code && <p className="text-[13px] text-textTC">Error Code : <span className="text-red-500">{status?.code}</span></p>}
          ...
          {status?.verified && (
            <div className="flex flex-col gap-4">
              <p className="text-[18px] text-textC">Your domain is live <Link href={`${HTTP}${store?.customDomain}`} target={'blank'} className="text-[#386ec5] hover:opacity-80">{store?.customDomain}</Link></p>
