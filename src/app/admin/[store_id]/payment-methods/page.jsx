...
const PaymentMethods = () => {
    ...
    useEffect(() => {
        console.log(validationErrors, "☠️☠️");
    }, [validationErrors])
                ...
                {PaymentMethodsStructure.map((method) => (
                    <div key={method.key} onClick={() => handleEdit(method.key)} >
                        <CustomCard
                            ...
                            className={`rounded-none border-x-0 border-b-0 border-t !py-5 hover:bg-[#fcfcfc] cursor-pointer ${method.disabled && 'opacity-60 cursor-not-allowed pointer-events-none'} ${editVisibleKey === method.key && 'bg-[#fcfcfc]'}`}
                            title={method.label}
                            subText={
                                method.subLable ? (
                                    method.subLable
