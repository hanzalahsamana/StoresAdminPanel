    ...
    <>
      <BackgroundFrame className="h-[calc(100vh-60px)]">
        {store?.subscriptionId?.status !== 'active' ? (
          ...
          <div className="bg-white mx-auto shadow-lg rounded-2xl p-8 max-w-md w-full text-center border border-gray-200">
            <div className="flex flex-col items-center gap-4">
              ...
              <h2 className="text-2xl font-bold text-textC">Subscription Active 🎉</h2>
              <p className="text-textTC">
                Your subscription is <span className=" text-green-600">active</span> and ready to use.
              </p>
              <p className="text-textC font-medium">
                Expiration Date:{' '}
                <span className="text-primary font-semibold">{store?.subscriptionId?.subsEnd ? new Date(store?.subscriptionId?.subsEnd).toLocaleDateString() : 'N/A'}</span>
              </p>
            </div>
          </div>
        )}
