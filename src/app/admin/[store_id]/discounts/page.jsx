...
const Discount = () => {
  ...
  return (
    <BackgroundFrame>
      <ActionCard
        label={'Discounts'}
        icon={<ImgToIcon url={'https://img.icons8.com/external-gradient-design-circle/66/external-discounts-activities-gradient-design-circle.png'} className={'!w-[50px]'} />}
        className="w-full"
        actionPosition="top"
        actions={
          <>
            <Button label={'Add New Discount'} size="small" action={() => setIsOpen(true)} />
          </>
        }
      ></ActionCard>
            ...
            >
              <div className="flex justify-between w-full">
                ...
                <div className="flex items-end flex-col gap-3 text-textTC text-[15px]">
                  ...
                  <p>{discount?.discountType === 'global' ? 'Not applicable in global' : discount?.access}</p>

                  <p>{discount?.amountType}</p>
                  <p>{discount?.amountType === 'percent' ? `${discount?.amount}%` : `${discount?.amount}Rs`}</p>
                  <p>{discount?.minOrderAmount || 0}</p>
                  <p>{discount?.discountType === 'global' ? 'Not applicable in global' : discount?.usageLimit ?? 'Unlimited'}</p>
                  <p>{discount?.discountType === 'global' ? 'Not applicable in global' : discount?.usagePerUser ?? 'Unlimited'}</p>
                  <p>{discount?.description || 'N/A'}</p>
                  <p>{discount?.expiryDate ? moment(discount?.expiryDate).format('YYYY-MM-DD HH:mm') : 'No Date'}</p>
                  <p className="flex items-center gap-2">
                    <StatusCard
                      label={new Date(discount?.expiryDate) <= new Date() ? 'Expired' : discount?.isActive ? 'Active' : 'Inactive'}
                      status={new Date(discount?.expiryDate) >= new Date() && discount?.isActive}
                    />
                  </p>
                </div>
              </div>
      ...
      <AddEditDiscountModal isOpen={isOpen} setIsOpen={setIsOpen} updatedDiscount={updatedDiscount} setUpdatedDiscount={setUpdatedDiscount} />
