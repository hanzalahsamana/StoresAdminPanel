...
const DropDown = ({
    ...
    const showCreateOption =
        wantsCustomOption &&
        searchTerm?.trim() &&
        !options.find((opt) => opt.label === searchTerm.trim());

    const menu = createPortal(
        <div
            ref={menuRef}
            className={`absolute top-[0] mt-[4px] w-[200px] bg-backgroundC box-content border rounded-md overflow-hidden shadow-md z-[10000] transition-all duration-150 ease-in-out overflow-y-auto customScroll ${visible ? "max-h-[150px] border" : "max-h-0 border-none"}`}
        ...
        >
            ...
            {options.map((option, index) => (
                <div
                    key={index}
                    className={`px-3 py-2 text-sm cursor-pointer ${selectedOption === option.value
                        ? "bg-[#e2e2e4] text-gray-700"
                        : "hover:bg-gray-100"
                        }`}
    ...
    return (
            <div className="flex w-max min-w-[200px]">
                <FormInput
                    error={error}
                    value={searchTerm}
                    ...
                    suffix={
                        <span
                            className={`text-[20px] px-2 transition-all ${!visible ? "rotate-180" : "rotate-0"
                                }`}
                        >
                            <IoMdArrowDropup />
                        </span>
