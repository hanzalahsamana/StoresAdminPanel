...
const normalizeOptions = (arr = []) => arr.map((item) => (typeof item === 'string' ? { label: item, value: item } : item));
...
const MultiSelectDropdown = ({
  ...
  const filteredOptions = useMemo(() => {
    return availableOptions.filter((option) => option.label.toLowerCase().includes(searchTerm.toLowerCase()) && !selectedOptions.includes(option.value));
  ...
  const showCreateOption =
    searchTerm.trim() &&
    !selectedOptions.some((val) => val.toLowerCase() === searchTerm.trim().toLowerCase()) &&
    !availableOptions.some((opt) => opt.label.toLowerCase() === searchTerm.trim().toLowerCase()) &&
    wantsCustomOption;

  return (
    <div className="w-full flex flex-col gap-1" ref={containerRef}>
      {label && <label className="text-[14px] font-medium text-textC mb-1 block">{label}</label>}
      <div ref={dropdownRef} className={`relative w-full ${className}`}>
        ...
        >
          {resolvedSelectedOptions.map((option, index) => (
            <span key={index} className="bg-gray-200 text-[10px] flex items-center gap-1 px-[4px] py-[2px] rounded-md">
              {option.label}
              <button
                onClick={(e) => {
                  e.stopPropagation();
          ...
          />
          <span className={`ml-auto mr-1 text-textTC text-[20px] transition-all ${isOpen ? 'rotate-0' : 'rotate-180'}`}>
            <IoMdArrowDropup />
          </span>
        </div>
        ...
        >
          ...
          {filteredOptions.length ? (
            filteredOptions.map((option, index) => (
              <div key={index} onClick={() => handleSelect(option)} className="px-3 py-2 flex justify-between text-sm cursor-pointer hover:bg-gray-100">
                {option.label}
                <div>{option?.icon}</div>
              </div>
            ))
