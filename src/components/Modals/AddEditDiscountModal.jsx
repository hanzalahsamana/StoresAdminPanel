...
const AddEditDiscountModal = ({ isOpen, setIsOpen, updatedDiscount = null, setUpdatedDiscount }) => {
  ...
  useEffect(() => {
    if (updatedDiscount) {
      setForm({
        ...
        expiryDate: updatedDiscount?.expiryDate ? formatDateTimeLocal(updatedDiscount.expiryDate) : formatDateTimeLocal(new Date()),
  ...
  const validate = () => {
    ...
    if (!form.amount) {
      ...
      if (form.amountType === 'percent' && amount > 100) errs.amount = 'Percentage cannot exceed 100';
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (updatedDiscount) {
        ...
        await addDiscount(data, currUser?.token, store_id);
  ...
  return (
    <Modal isOpen={isOpen} setIsOpen={setIsOpen} extraFuntion={functionBeforeClose}>
      <ActionCard
        ...
        actionPosition="top"
        actions={<Button className="mr-[20px]" size="small" action={handleSave} label={updatedDiscount ? 'Update Discount' : 'Save Discount'} loading={loading} />}
      >
