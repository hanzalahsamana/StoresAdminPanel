      ...
      <div className="flex flex-wrap gap-5 md:gap-0">
        <div className="w-full md:w-3/5 space-y-5 md:pr-[10px]">
          <CustomCard icon={<CgDetailsMore size={20} color="#000" />} title="Product Details" className="break-inside-avoid flex-none !p-4 pt-3">
            <FormInput name="name" label="Product Name" placeholder="e.g. Nike Air Zoom Pegasus 40" layout="label" value={formData.name || ""} onChange={(e) => handleChange(e.target.name, e.target.value)} error={errors.name} />
            <FormInput name="pronounce" label="Pronounce" placeholder="e.g. Piece , Box" layout="label" value={formData.pronounce || ""} onChange={(e) => handleChange(e.target.name, e.target.value)} error={errors.pronounce} />
        ...
        <div className="w-full md:w-2/5 space-y-5 md:pl-[10px]">
          ...
          <CustomCard icon={<FaSearchengin />} title="Meta Info" info={"Meta Info helps search engines understand your product better. 'Meta Title' is the short headline shown in search results, and 'Meta Description' is a short summary that encourages people to click your product. It's optional but useful for improving visibility on Google."} className="break-inside-avoid flex-none !p-4 pt-3">
            <FormInput name="metaTitle" label="Meta Title" layout="label" placeholder="e.g. Stylish Men's Running Shoes" value={formData.metaTitle || ""} onChange={(e) => handleChange(e.target.name, e.target.value)} error={errors.metaTitle} />
            <FormInput name="metaDescription" label="Meta Description" layout="label" placeholder="e.g. High-performance shoes with breathable mesh ..." value={formData.metaDescription || ""} onChange={(e) => handleChange(e.target.name, e.target.value)} error={errors.metaDescription} />
          </CustomCard>
        </div>
      </div>
    </div>
