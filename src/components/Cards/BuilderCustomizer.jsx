  ...
  const handleInputChange = (key, value) => {
    ...
    setCustomizePageData((prev) => {
      if (updatedActiveSection.type === 'header') {
        ...
        const updatedSections = prev.sections.map((section) => (section._id === updatedActiveSection._id ? updatedActiveSection : section));
        return {
          ...prev,
        ...
        {!activeSection ? (
          <div className="border-b h-full ">
            <div className="flex justify-start items-center gap-1  h-[55px] text-[#323232] border-b p-3">
              <p className="text-[18px] font-medium text-gray-700 flex flex-1 items-center gap-1 ">
                <CiFileOn /> {customizePageData?.name}
              </p>
              <PopupMenu2
                data={pages?.map((page) => ({
                  icon: <CiFileOn />,
                  name: page?.name,
                  action: () => {
                    router.push(`./customize?page=${page?.slug}`);
                  },
                }))}
                  ...
                  {(section, index, { provided, snapshot }) => (
                    <div key={section?._id}>
                      {customizePageData?.isEditable && (
                        <SectionAddAnimationLine selectedOrder={selectedOrder} setSelectedOrder={setSelectedOrder} index={index} setIsOpen={setIsOpen} />
                      )}
                      <SectionItemCard
                        provided={provided}
                        snapshot={snapshot}
