          ...
          <tr className="bg-secondaryC border-secondaryC border-b-[2px] text-primaryC uppercase text-sm">
            {isCheckbox && (
              <td className="p-2 text-left">
                <Checkbox
                  setChecked={(e) => handleCheckboxSelectAll(e)}
                  checked={data.length > 0 && Array.isArray(selectedData) && data.every((item) => selectedData.some((id) => id === item._id))}
                  label=""
                />
              </td>
            )}
          ...
          {loading ? (
            ...
            data.map((row, rowIndex) => (
              ...
              >
                {isCheckbox && (
                  <td className="p-2">
                    <Checkbox setChecked={(e) => handleCheckbox(row._id)} checked={selectedData.includes(row._id)} label="" />
                  </td>
                )}
                {columns.map((col) => {
                    ...
                    <td key={col.key} className="p-2 text-[14px] text-textC">
                      {col.type === 'image' ? (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <IconButton icon={<IoCopyOutline />} className="text-primaryC !text-[16px]" action={() => copyToClipboard(value)} />
                        </div>
                      ) : col.type === 'stock' ? (
                        <p
                          className={`flex gap-1 py-[2px] justify-start text-nowrap  items-center text-sm w-[125px] px-[10px] rounded-2xl ${
                ...
                {!!Object.keys(actions)?.length && (
                  <td className="p-3 space-x-2 flex justify-end">
                    {actions.edit && <IconButton icon={<MdOutlineEdit />} className="text-blue-500" action={() => actions.edit(row)} />}
                    {actions.delete && <IconButton icon={<AiOutlineDelete />} className="text-red-500" action={() => actions.delete(row)} />}
                    {actions.redirect && (
                      <div onClick={() => actions.redirect(row)} className="flex cursor-pointer text-primaryC gap-1 hover:opacity-65">
                        view
                        <IconButton icon={<VscArrowRight />} className="text-primaryC" />
                      </div>
                    )}
                  </td>
                )}
