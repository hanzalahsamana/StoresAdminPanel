  ...
  return (
    <header className="bg-backgroundC border-b border-gray-300 px-3 md:px-10 text-textC flex justify-between items-center fixed w-full z-[110] top-0 left-0 h-[60px]">
      ...
      <div className="flex items-center gap-2 ">
        <a
          target="_blank"
          href={`${HTTP}${store?.subDomain}.${Base_Domain}`}
        ...
        <PopupMenu />
        <PopupMenu2 />
        {store?.subscriptionId?.status === 'active' && <Button label="Reward" size="small" variant="black" icon={<IoGiftOutline />} action={() => setReferralModal(true)} />}
      </div>
      {store?.subscriptionId?.status === 'active' && <ReferralModal setIsOpen={setReferralModal} isOpen={referralModal} />}
    </header>
  );
}
