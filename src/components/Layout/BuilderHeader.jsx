    ...
    <div className="bg-white h-[60px] w-full flex justify-between items-center px-4 shadow-sm border-b z-10">
      <div className="flex items-center gap-3">
        ...
        {currentMode === 'draft' ? (
          <p
            ...
            className="!text-[22px] flex items-center gap-2 bg-gray-100 text-gray-700 p-2 rounded-md cursor-pointer  "
            data-tooltip-id={'preview'}
            data-tooltip-content={'Discard Back To Published Version'}
          >
            <LuHistory />
          </p>
        ) : (
      ...
      <div className="flex gap-2 items-center">
        ...
        <IconButton
          icon={<BiRedo />}
          tooltipLabel={'Redo'}
          className="!text-[22px] flex flex-col hover:bg-gray-100 text-gray-300 p-2 rounded-md hover:opacity-100 !cursor-not-allowed "
        >
          {' '}
        </IconButton>
        <Button size="small" label="Save As Draft" action={handleSaveDraftPage} loading={loading?.draftLoading} variant="black" active={isModified} />
        <Button
          size="small"
          ...
          icon={<VscLiveShare />}
          active={isModified || currentMode === 'draft'}
        />
      </div>
