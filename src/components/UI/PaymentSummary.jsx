...
function PaymentSummary({ children, className, totalProductCost, globalDiscount, couponDiscount, subTotal, shipping, tax, total }) {
  return (
    <div className={`flex w-full gap-2 text-[var(--tmp-ltxt)] bg-[var(--tmp-pri)] rounded-md border border-[var(--tmp-lBor)] py-4 px-4 ${className}`}>
      <div className="flex-col w-full">
        <p className="w-full flex justify-between text-[12px] font-semibold border-b border-[var(--tmp-lBor)] pb-2">
        ...
        <p className="w-full flex justify-between text-[12px] border-t border-[var(--tmp-lBor)] pt-2">
          <span>Sub Total</span>
        ...
        <p className="w-full flex justify-between text-[12px] pt-2">
          ...
          <span>{shipping > 0 ? `Rs ${formatAmount(shipping)}` : <em className="italic">Free Shipping</em>}</span>
        </p>

        <p className="w-full flex justify-between text-[12px] mt-[6px]">
          <span>Tax</span>
          <span>{tax > 0 ? `Rs ${formatAmount(tax)}` : <em className="italic">No Tax</em>}</span>
        </p>

        <p className="w-full flex justify-between text-[20px] text-[var(--tmp-txt)] font-semibold mt-[10px] border-t border-[var(--tmp-lBor)] pt-2">
          <span>Total</span>
