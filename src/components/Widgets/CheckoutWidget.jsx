...
const CheckoutWidget = forwardRef(({ sectionData = {}, ...rest }, ref) => {
  const { globalLogo, checkoutLogo, buttonText = '', contactFields, couponInput, style } = sectionData;
  ...
  return (
    <div className="grid grid-cols-2 w-full flex-col-reverse md:flex-row" ref={ref} {...rest}>
      ...
      <div className="h-screen bg-[var(--tmp-acc)] w-full px-8 py-6 flex justify-start sticky top-0">
        <div className="max-w-[500px] w-full flex flex-col h-full">
          ...
          <PaymentSummary
            ...
            total={total}
          >
            {couponInput && <ApplyCoupon email={'abc@gmail.com'} totalProductCost={totalProductCost - globalDiscount?.discountAmount || 0} setCouponDiscount={setCouponDiscount} />}
            <Button
              label={buttonText || 'Proceed To Payment'}
              loading={loading}
              action={handleSubmit}
              size="small"
              variant="black"
