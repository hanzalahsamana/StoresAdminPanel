...
const Catalog = forwardRef(({ sectionData = {}, ...rest }, ref) => {
    ...
    const { data, isLoading, error } = useSwrFetch(
        `${BASE_URL}/${store?._id}/getProducts${queryString}`
    );
    ...
    return (
        ...
        ref={ref}
        {...rest}
        className="flex justify-center items-center w-full bg-[var(--tmp-pri)]"
        >
        <BlurLoader isVisible={isLoading} />;
            <div className="flex flex-col gap-5 w-full p-6">
                <div className="flex justify-between items-end">
                    <h1 className="text-[45px] text-[var(--tmp-txt)] font-semibold text-center">
                        {heading || "Catalog"}
                    </h1>
                        ...
                        {(variantsFilters || standardFilters) && (
                            <>
                                ...
                                <button
                                    ...
                                    className="px-7 text-lg py-2 bg-transparent !font-thin border flex gap-2 items-center text-black"
                                >
                                    <VscSettings className="text-lg" /> Filters
                                </button>
                            </>
                        )}
                    </div>
