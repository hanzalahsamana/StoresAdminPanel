...
const ProductAddForm = () => {
    ...
    const handleImageChange = (event) => {
        ...
        const newImages = files.filter((file) =>
            !selectedImages.some((image) => image.name === file.name)
        );
    ...
    const validateForm = () => {
        const newErrors = {};
        const { name, brand, originalPrice, discountedPrice, discount, collectionName, type, size } =
            formData;

        if (!name) newErrors.name = "name is required";
        if (!brand) newErrors.brand = "brand Name is required";
        if (!originalPrice || originalPrice == 0) newErrors.originalPrice = "original Price is required";
        ...
        if (selectedImages.length > 4 || selectedImages.length < 2) newErrors.image = "Please select max 4 and min 2 images.";
    ...
    return (
        <div className="w-full flex justify-center p-[30px]">
            <div className="max-w-[1300px] w-full">
                <form onSubmit={handleSubmit} className="flex flex-wrap"  encType="multipart/formdata">
                    <div className="w-full">
                        <h3 className="text-[28px] font-semibold my-[5px]">Product Detail</h3>
                    ...
                    <div className="flex gap-2 items-center mb-[20px] relative">
                        {errors.image && <p className=" absolute bottom-[-25px] text-red-500 text-[10px]">{errors.image}</p>}
                        ...
                        <label className="flex w-[100px] h-[100px] justify-center items-center border-2 border-dashed border-gray-300 rounded-lg p-3 cursor-pointer hover:bg-gray-100 transition duration-300">
                            <span className="text-gray-600 text-[10px]">Upload Images<br /> (min 2) (max 4)</span>
                            <input
                                type="file"
                                accept="image/*"
                                multiple
