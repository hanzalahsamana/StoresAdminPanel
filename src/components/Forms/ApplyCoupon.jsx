    ...
    <form onSubmit={handleApply} className="flex flex-col gap-2 mt-[10px]">
      <div className="flex items-end gap-3">
        <FormInput
          ...
          isStore={true}
        />
        <Button type="submit" loading={loading} variant="black" size="small" label="Apply Coupon" className="bg-[var(--tmp-sec)] text-[var(--tmp-wtxt)] " />
      </div>
      {message && <p className="text-[12px] mt-[-5px] text-red-500">{message}</p>}
    </form>
  );
};
