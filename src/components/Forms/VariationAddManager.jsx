...
const VariationAddManager = ({
    ...
    const handleUpdateVariationName = (id, name) => {
        const updated = variationData.map(variation =>
            variation.id === id ? { ...variation, name } : variation
        );
        setVariationData(updated);
        const validation = validateVariation(updated.find(v => v.id === id), updated);
        setErrors(prev => ({ ...prev, [id]: validation }));
    };

    const handleUpdateVariationOptions = (id, options) => {
        const updated = variationData.map(variation =>
            variation.id === id ? { ...variation, options } : variation
        );
        setVariationData(updated);
        const validation = validateVariation(updated.find(v => v.id === id), updated);
        setErrors(prev => ({ ...prev, [id]: validation }));
                                    ...
                                    <div className="w-full">
                                        ...
                                        <div className="flex justify-between mt-[15px]">
                                            <Button
                                                ...
                                                size="small"
                                                action={() => handleDelete(variation.id)}
                                            />
                                            <Button
                                                label="Done"
                                                variant="black"
                                                size="small"
                                                action={() => handleDone(variation.id)}
                                            />
                                        </div>
                                    </div>
                                )}
